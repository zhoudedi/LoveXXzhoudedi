<?php
/**
 * Typecho 祝福发布、祝福被回复、祝福审核通过通知、发送给用户版、加找回密码插件
 *
 * @package LoveXXzhoudedi
 * @author  小周
 * @version 1.0.0
...
class LoveXXzhoudedi_Plugin implements Typecho_Plugin_Interface
{
    /**
     * 激活插件
     *
    ...
    static public function activate()
    {
        ...
        // 找回密码动作添加
        Helper::addAction('LoveXXzhoudedi', 'LoveXXzhoudedi_Action');
        // 找回密码路由添加
        Helper::addRoute('LoveXXzhoudedi_forget', '/LoveXXzhoudedi/forget', 'LoveXXzhoudedi_Action', 'forget');
        Helper::addRoute('LoveXXzhoudedi_reset', '/LoveXXzhoudedi/reset', 'LoveXXzhoudedi_Action', 'reset');
    }
    ...
    {
        // 删除动作
        Helper::removeAction('LoveXXzhoudedi');
        // 删除路由
        Helper::removeRoute('LoveXXzhoudedi_forget');
        Helper::removeRoute('LoveXXzhoudedi_reset');
    }
    ...
    static public function configCheck(array $settings)
    {
        ...
        // 获取插件配置
        $plugin = $options->plugin('LoveXXzhoudedi');
        // 是否启用配置验证
    ...
    static public function sendMail($commentId, $isApproved = FALSE)
    {
        ...
        if ( $isApproved ) {
            // 邮件标题
            $data['subject'] = _t('您在 [' . trim($options->title) . ']  发表的祝福已通过审核！');
            // 读取模板
            ...
            if ( !is_null($parentComment) ) {
                // 标题
                $data['subject'] = _t('您在 [' . trim($options->title) . '] 发表的祝福有了新的回复！');
                // 读取模板
                ...
                // 标题
                $data['subject'] = _t('收到您在 [' . trim($options->title) . ']  发表的祝福！');
                // 读取模板
    ...
    static public function sendCloud($data)
    {
        ...
        $flag = TRUE;
        // 获取插件配置
        $plugin = Helper::options()->plugin('LoveXXzhoudedi');
        // 如果开启了Debug
        ...
        if ( in_array('enable', $plugin->public_forget) ) {
            ...
            if ( preg_match('/\/login\.php/i', $pathinfo) ) {
                ...
                <script>
                    ...
                    forget.href = '<?php echo Typecho_Common::url('/action/LoveXXzhoudedi?forget', Helper::options()->index);?>';
